...
class NotionExtractor(BaseExtractor):
    ...
    def __init__(
            ...
            tenant_id: str,
            document_model: Optional[KnowledgeDocument] = None,
    ...
    def update_last_edited_time(self, document_model: KnowledgeDocument):
        ...
        with self.postgres.session as session:
            ...
            update_params = {
                KnowledgeDocument.data_source_info: json.dumps(data_source_info)
            }

            KnowledgeDocument.query.filter_by(id=document_model.id).update(update_params)
            session.commit()
