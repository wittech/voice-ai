 ...
 *
 * it will list all the file and folder from your google drive you decide what to select that will be used for you knwoledge base
 */
export const GoogleDriveKnowledgeFileListing: FC<KnowledgeFileListingProps> =
  memo(({ toolProvider, className, onChangeContents }) => {
    /**
     * connector file context
     */
    const ctx = useContext(ConnectorFileContext);

    // user and token, project
    ...
    // type of pages found in connect source
      ...
      selectedFiles.forEach(y => {
        ctx.allFiles
          ...
          .forEach(x => {
            ...
            let name = x.getFieldsMap().get('title')?.getStringValue();
    ...
    /**
     * on success of revieving the files
     */
