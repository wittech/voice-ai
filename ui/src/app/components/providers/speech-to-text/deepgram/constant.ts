...
export const GetDeepgramDefaultOptions = (current: Metadata[]): Metadata[] => {
  ...
  addMetadata('listen.language', 'multi', value =>
    DEEPGRAM_SPEECH_TO_TEXT_LANGUAGE().some(l => l.code === value),
  );

  // Set model
  addMetadata('listen.model', 'nova-3', value =>
    DEEPGRAM_SPEECH_TO_TEXT_MODEL().some(m => m.id === value),
  );
...
export const ValidateDeepgramOptions = (options: Metadata[]): boolean => {
  ...
  if (
    !languageOption ||
    !DEEPGRAM_SPEECH_TO_TEXT_LANGUAGE().some(
  ...
  if (
    !modelOption ||
    !DEEPGRAM_SPEECH_TO_TEXT_MODEL().some(
