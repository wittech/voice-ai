...
export const GetVertexAiTextProviderDefaultOptions = (
  current: Metadata[],
): Metadata[] => {
  const mtds: Metadata[] = [];
  const keysToKeep = [
...
export const ValidateVertexAiTextProviderDefaultOptions = (
  options: Metadata[],
): string | undefined => {
  ...
  if (
    !modelIdOption ||
    !VERTEXAI_MODEL().some(model => model.id === modelIdOption.getValue())
  ) {
