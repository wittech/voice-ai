...
import { ArrowDownLeft, ArrowUpRight } from 'lucide-react'; // Importing React Lucide icons
...
> = ({ direction, source, size = 'medium', withLabel = true }) => {
    ...
    inbound: {
      ...
      icon: (
        <ArrowDownLeft
          ...
          strokeWidth={1.5}
        />
      ), // Using Lucide ArrowL strokeWidth={1.5}eft
    },
    outbound: {
      ...
      label: 'Outbound',
      icon: <ArrowUpRight className="w-3 h-3" strokeWidth={1.5} />, // Using Lucide ArrowRi strokeWidth={1.5}ght
    },
  };
  ...
  const sourceConfig = {
    ...
    sdk: {
      ...
      icon: <Code className="w-3 h-3" strokeWidth={1.5} />,
      label: 'SDK',
    },
    'web-plugin': {
    ...
    debugger: {
      ...
      icon: <Bug className="w-3 h-3" strokeWidth={1.5} />,
      label: 'Debugger',
    },
    'rapida-app': {
      ...
      icon: <RapidaIcon className="w-3 h-3" strokeWidth={1.5} />,
      label: 'Rapida App',
    },
    'node-sdk': {
      bgColor: 'bg-green-100 dark:bg-green-900/30',
    ...
    'java-sdk': {
      ...
      icon: <Coffee className="w-3 h-3" strokeWidth={1.5} />,
      label: 'Java SDK',
    },
    'php-sdk': {
    ...
    'twilio-whatsapp': {
      ...
      icon: <WhatsappIcon className="w-3 h-3" strokeWidth={1.5} />,
      label: 'WhatsApp',
    },
  };
  ...
  const sizeClasses = {
    small: 'text-xs/6 px-2 py-0.5',
    medium: 'text-sm/6 px-2.5 py-1',
    large: 'text-base/6 px-3 py-1.5',
  };

  return (
    <span
      className={`inline-flex items-center rounded-[2px] ${config.bgColor} ${config.textColor} font-medium border-[0.1px] ${config.borderColor}`}
    >
      <span
        className={cn(config.borderColor, sizeClasses[size], 'flex relative')}
      >
        {sourceCfg.icon}
        {config.icon}
      </span>
