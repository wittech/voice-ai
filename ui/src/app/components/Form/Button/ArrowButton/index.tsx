 ...
 * @returns
 */
export function ArrowButton(props: ArrowButtonProp) {
  /**
  ...
  return (
    <Button
      {...props}
      disabled={props.loading || props.disabled || isBlocking()}
      className={cn(
...
export function ArrowBorderButton(props: ArrowButtonProp) {
  /**
   * for disabling and loading
   */
  const { isBlocking } = useRapidaStore();
  ...
  return (
    <BorderButton
      {...props}
      disabled={props.loading || props.disabled || isBlocking()}
      className={cn(isBlocking() ? 'opacity-80' : 'opacity-100')}
    >
      <span className="font-medium text-sm">{props.label}</span>
      {isBlocking() ? (
        <div role="status" className="ml-1">
